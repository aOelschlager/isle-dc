...
local:
	...
	if [ -z "$$(ls -A $(CURDIR)/codebase)" ]; then \
		docker container run --rm -v $(CURDIR)/codebase:/home/root $(REPOSITORY)/nginx:$(TAG) with-contenv bash -lc 'git clone -b install-profile https://github.com/Islandora-Devops/islandora-sandbox.git /tmp/codebase; mv /tmp/codebase/* /home/root;'; \
	fi
	docker-compose up -d
